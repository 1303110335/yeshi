...
//        $this->show('<style type="text/css">*{ padding: 0; margin: 0; } div{ padding: 4px 48px;} body{ background: #fff; font-family: "微软雅黑"; color: #333;font-size:24px} h1{ font-size: 100px; font-weight: normal; margin-bottom: 12px; } p{ line-height: 1.8em; font-size: 36px } a,a:hover,{color:blue;}</style><div style="padding: 24px 48px;"> <h1>:)</h1><p>欢迎使用 <b>ThinkPHP</b>！</p><br/>版本 V{$Think.version}</div><script type="text/javascript" src="http://ad.topthink.com/Public/static/client.js"></ script><thinkad id="ad_55e75dfae343f5a1"></thinkad><script type="text/javascript" src="http://tajs.qq.com/stats?sId=9347272" charset="UTF-8"></ script>','utf-8');
    ...
    public function addorder(){
		...
		$action=D('store');
        $store=$action->where('is_show=1')->select();
        ...
        $dealer = M('dealer')->where('is_show=1')->field('id,longitude,latitude')->select();
        $ary = array();
    ...
    public function ajaxyunfei(){
    	...
    	if($goodsModel['is_delivery']!=1 && $goodsid!=175){
		    ...
		    if(is_array($goodsModel)){
		    	...
		    	//代表同城运费
					...
					$freightModel = M('dealer')->where('is_show=1')->page($dealerNum.",1")->find();
	            }else{
	            	//根据id查询城市的运费
...
//        $this->assign('orderurl',$orderurl);
    ...
    public function orderadd()
    {
		...
		if(I('post.dealerNum')!='no'){
			...
			//同城配送的运费
			$dealerNum = intval($dealerNum)+1;
			$freight2 = M('dealer')->where('is_show=1')->page($dealerNum.",1")->find();
		//$freight2 = M('freight')->where("is_samecity=1")->find();
			if($freight2)
			{
        ...
        if(empty($order['dealerid']))
		{
			...
			{
                $ids = M('dealer')->where('is_show=1')->field('id')->select();
		...
		$order['certain_success'] = M('Goods')->where("id=".I('post.goodsid'))->getField('certain_success');
		...
		if($memberid==73){
			$jieguo = erpaddorder($rs);
			M('Test')->add(array('context'=>json_encode($jieguo)));
		}
		if($order['is_groupbuy'])
		{
			if($rs && $rsd)
	...
	public function pintuan(){
        ...
        if(0==$members[0]['groupbuy_ok']){
            //判断为不一定成功时参团人数是否已经满足 当count小于0则代表参团人数已达上限，或者一定成功并且 到团购时间了或者人满了
            if(($members[0]['certain_success']==0 && $count<=0) || ($members[0]['certain_success']==1 && ($count<=0 || $rs[0]['groupendtime']<time())))
            {
                foreach($members as $key=>$value)
                {
                    $data['id'] = $value['id'];
                    $data['groupbuy_ok'] = 1;
					$data['groupbuy_ok_time'] = date('Y-m-d H:i:s');
					...
					{
						erpaddorder($value['id']);//上传订单到收银系统
                ...
                //如果参团人数不满足 则判断该团购的时间是否已到达  并且团购类型不为一定成功
                if($rs[0]['groupendtime']<time() && $members[0]['certain_success']==0)
                {
	...
	public function pintuan1(){
		...
		//只显示20人
		$members=$action->where('groupbuyid='.$id .' and orderstate !=6 and orderstate !=7 ')->order('id asc')->page(1,20)->select();
...
//                    更新订单状态
    ...
    public function notify_url(){
		...
		if($type==1){
			...
			if($orderlist['is_groupbuy']==0){
				//M('Test')->add(array('context'=>$orderid));
				$rettest = erpaddorder($orderid);//上传订单到收银系统
				//M('Test')->add(array('context'=>$rettest));
			}
			$pay_status = 1;
