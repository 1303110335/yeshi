...
function sendmessage($type,$phone,$phoneCode='',$orderid=''){
	...
	{
		case 1:
		$msg = "您的手机验证码为：".$code.",为保证验证安全，请在半小时之内进行验证。" ;
		break;	
		
		case 2:
		$msg = "您此次购买商品的订单号为：".$orderid.",提货码为：".$phoneCode.",门店提货以此提货码为凭据，请牢记！" ;
		break;
	...
	$post_data = array();
	$post_data['account'] = '003522';   //帐号
	$post_data['pswd'] = 'Sd123456';  //密码
...
function panduanguanzhu($member_id=0,$openid=0){
	...
	if($member_id){
		$openid = M('Member')->where("id=".$member_id)->getField('weixin_openid');
...
function erpaddorder($orderlistid){
	...
	$orderlist = M('Orderlist')->alias('a')->join("left join ys_store b on a.dealerid=b.dealerid")->field('a.id,a.userid,a.storeid,a.orderno,a.consignee,a.dealerid,a.province,a.city,a.district,a.address,a.telephone,a.addtime,a.total_price,a.shipping_fee,a.pay_fee,a.paytime,b.erpstoreid')->where("a.id=$orderlistid")->find();
	...
	if($orderlist){
		...
		if(is_array($orderdetaillist)){
			...
			if($result['data']){
				...
				M('Orderlist')->where("id=$orderlist[id]")->save(array('barcode'=>$filename,'online_order_fid'=>$result['data']));
			}
		}
	}
	return $result;
